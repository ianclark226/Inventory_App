...
class InventoryGroupSerializer(serializers.ModelSerializer):
    ...
    craeted_by_id = serializers.CharField(write_only=True, required=False)
...
class InventorySerializer(serializers.ModelSerializer):
    crated_by = CustomUserSerializer(read_only=True)
...
class InvoiceSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if not invoice_item_data:
            raise Exception("You need to provide at lease one invoice item")
